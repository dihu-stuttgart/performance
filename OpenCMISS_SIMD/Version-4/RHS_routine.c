                      ...
                      double * restrict Gamma, double * restrict StimValue, double * restrict L_hs, clock_t * restrict handle_array, double * restrict portions_array, int* usedRamSize_pnt)
{
 ...
 TMP70 = malloc(sizeof(double));
 TMP71 = malloc(sizeof(double));
 TMP72 = malloc(sizeof(double));
 TMP73 = malloc(sizeof(double));
...
/* Constant S_i */
CONSTANTS[46] = 10;
TMP73[0] = CONSTANTS[46]*CONSTANTS[46];
/* Constant J_NaK_bar */
...
/* Constant f */
CONSTANTS[56] = 0.2;
TMP70[0] = CONSTANTS[56]*CONSTANTS[56];
TMP71[0] = TMP70[0]*CONSTANTS[56];
TMP72[0] = TMP70[0]*TMP70[0];
/* Constant alpha1 */
CONSTANTS[57] = 0.2;
/* Constant K */
CONSTANTS[58] = 4.5;
...
CONSTANTS[103] = 60;
/* Constant V_o_Eqn */
CONSTANTS[104] =  0.950000*CONSTANTS[65]* 3.14159265358979*CONSTANTS[66]*CONSTANTS[66];
/* Constant V_SR_Eqn */
CONSTANTS[105] =  0.0500000*CONSTANTS[65]* 3.14159265358979*CONSTANTS[66]*CONSTANTS[66];
/* Constant V_1_Eqn */
...
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   ...
   TMP18[cell] = 8571.00/(0.200000+ 5.65000*((X0[cell]+CONSTANTS[48])*(X0[cell]+CONSTANTS[48])*0.0001));
}
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized */
   /* dS_Eqn */
...
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   ...
   TMP23[cell] = 8571.00/(0.200000+ 5.65000*((X1[cell]+CONSTANTS[48])*(X1[cell]+CONSTANTS[48])*0.0001));
   }
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized */
   /* dS_t_Eqn */
...
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   ...
   X_t25[cell] =  3.00000*TMP11[cell]*X24[cell]+ -2.00000*TMP24[cell]*X25[cell]+( - CONSTANTS[54]*X25[cell])/TMP70[0]+ TMP70[0]*CONSTANTS[55]*X20[cell]+ -2.00000*TMP11[cell]*X25[cell]+ 3.00000*TMP24[cell]*X26[cell];
   }
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   /* dO_2_Eqn */
   ...
   X_t20[cell] = ( 3.00000*TMP11[cell]*X19[cell])/CONSTANTS[56]+ -2.00000*CONSTANTS[56]*TMP24[cell]*X20[cell]+( CONSTANTS[54]*X25[cell])/TMP70[0]+ - CONSTANTS[55]*TMP70[0]*X20[cell]+( -2.00000*TMP11[cell]*X20[cell])/CONSTANTS[56]+ 3.00000*CONSTANTS[56]*TMP24[cell]*X21[cell];
   }
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   /* dC_3_Eqn */
   ...
   X_t26[cell] =  2.00000*TMP11[cell]*X25[cell]+ -3.00000*TMP24[cell]*X26[cell]+( - CONSTANTS[54]*X26[cell])/TMP71[0]+ CONSTANTS[55]*TMP71[0]*X21[cell]+ - TMP11[cell]*X26[cell]+ 4.00000*TMP24[cell]*X27[cell];
   }
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   /* dO_3_Eqn */
   ...
   X_t21[cell] = ( CONSTANTS[54]*X26[cell])/TMP71[0]+ - CONSTANTS[55]*TMP71[0]*X21[cell]+( 2.00000*TMP11[cell]*X20[cell])/CONSTANTS[56]+ -3.00000*TMP24[cell]*CONSTANTS[56]*X21[cell]+( - TMP11[cell]*X21[cell])/CONSTANTS[56]+ 4.00000*CONSTANTS[56]*TMP24[cell]*X22[cell];
   }
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   /* dC_4_Eqn */
   ...
   X_t27[cell] =  TMP11[cell]*X26[cell]+ -4.00000*TMP24[cell]*X27[cell]+( - CONSTANTS[54]*X27[cell])/TMP72[0]+ CONSTANTS[55]*TMP72[0]*X22[cell];
   }
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   /* dO_4_Eqn */
   ...
   X_t22[cell] = ( TMP11[cell]*X21[cell])/CONSTANTS[56]+ -4.00000*CONSTANTS[56]*TMP24[cell]*X22[cell]+( CONSTANTS[54]*X27[cell])/TMP72[0]+ - CONSTANTS[55]*TMP72[0]*X22[cell];
   }
#pragma GCC ivdep
for(cell=1;cell<=N;cell++){/* vectorized */
  TMP24[cell] = CONSTANTS[35]*CONSTANTS[36];
}
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized */
   /* own */
...
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   /* g_IR_bar_Eqn */
   TMP37[cell] =  CONSTANTS[40]*((TMP36[cell]*TMP36[cell])/(CONSTANTS[42]+(TMP36[cell]*TMP36[cell])));
...
for (cell=1;cell<=N;cell++){/* vectorized */
   ...
   TMP38[cell] = 1.00000 - 1.00000/(1.00000+( CONSTANTS[43]*(1.00000+(TMP36[cell]*TMP36[cell])/CONSTANTS[42]))/((TMP73[0])*TMP38[cell]));
   }
//#pragma GCC ivdep
//for (cell=1;cell<=N;cell++){/* ##_R_E_P_L_A_C_E_D_### */
//   /* y_Eqn */
...
for (cell=1;cell<=N;cell++){/* vectorized */
   ...
   TMP47[cell] = 1.00000/(1.00000+ 0.120000*TMP47[cell]+ 0.0400000*TMP46[cell]*TMP41[cell]);
   }
//#pragma GCC ivdep
//for (cell=1;cell<=N;cell++){/* ##_R_E_P_L_A_C_E_D_### */
//   /* f1_Eqn */
...
for (cell=1;cell<=N;cell++){/* vectorized pow() */
   ...
   TMP56[cell] =  CONSTANTS[40]*((TMP55[cell]*TMP55[cell])/(CONSTANTS[42]+(TMP55[cell]*TMP55[cell])));
   }
#pragma GCC ivdep
for (cell=1;cell<=N;cell++){/* vectorized */
   /* own */
...
for (cell=1;cell<=N;cell++){/* vectorized */
   ...
   TMP57[cell] = 1.00000 - 1.00000/(1.00000+( CONSTANTS[43]*(1.00000+(TMP55[cell]*TMP55[cell])/CONSTANTS[42]))/((TMP73[0])*TMP57[cell]));
   }
//#pragma GCC ivdep
//for (cell=1;cell<=N;cell++){/* ###_R_E_P_L_A_C_E_D_### */
//   /* y_t_Eqn */
...
for (cell=1;cell<=N;cell++){/* vectorized */
   ...
   TMP66[cell] = 1.00000/(1.00000+ 0.120000*TMP66[cell]+ 0.0400000*TMP65[cell]*TMP60[cell]);
   }
//#pragma GCC ivdep
//for (cell=1;cell<=N;cell++){/* ###_R_E_P_L_A_C_E_D_### */
//   /* f1_t_Eqn */
