 ...
 int main(int argc,char **argv)
{
   ...
   //##################################################################################################
   // to measure the time for the simulation. Total time, and specific portions of it.
   struct timeval *tv0, *tv1;
   struct timezone *tz=NULL;
   tv0 = malloc(sizeof(struct timeval));
   tv1 = malloc(sizeof(struct timeval));
   clock_t start, end;
   int NumberOfPortions = 8, idx;
   clock_t *handle;
...
intermediate0 = malloc(sizeof(double)*NumberOfCells);
   ...
   if(T == 0){
      ...
      forwardEulerForAVX(dt_1D,K,
                      ...
                      intermediate0,parameter0,parameter1,NumberOfCells,handle,cpu_time_portions,&max_kB);
      end = clock();
      gettimeofday(tv1,tz);
