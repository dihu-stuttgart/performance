...
NumberGlobalYElements = 3       # number of finite elasticty elements in y-direction, alias y
NumberGlobalZElements = 3       # number of finite elasticty elements in z-direction, alias z

NumberOfNodesInXi1 = 741        # number of bioelectric elements (i.e. number nodes+1) per 3D FE element in direction of fibre (X direction), alias xi1
...
ElasticityDisabled = T          # disable the elasticity control loop

# ------------- numerics -------------------
# time steps
TimeStop = 2.0                 # total simulated time, note that the real simulated time is always a multiple of StimPeriod
ODETimeStep = 1e-4              # timestep size for 0D problem
PDETimeStep = 5e-4              # timestep size for 1D problem
...
FiringTimesFile = MU_firing_times_immediately.txt     # file that contains the time steps (rows) for each fibre (columns) when the fibre is stimulated (value 1 else 0)
