...
input_filename = "logs/log_10.csv"
...
df_gmres = df_gnu[df_gnu['activationSolver_solverType'].str.contains('gmres')]
df_cg = df_gnu[df_gnu['activationSolver_solverType'].str.contains('cg')]

print("n rows:")
print(len(df.index))
print("n GMRES:")
print(len(df_gmres.index))
print("n CG:")
print(len(df_cg.index))
...
def plot(df, title, columns_to_plot, mylabels=None):
  ...
  ax = means.plot(figsize=(13,7), y=columns_to_plot, title = "weak scaling", logx=True, logy=True, yerr=errors, marker='o')
...
mylabels=None
columns_to_plot = ["totalUsertime", "duration_init", "duration_bidomain", "duration_1D", "duration_0D", "duration_transfer_01D", ]
...
mylabels = ["total", "initialization", "3D model", "1D model", "0D model", "communication 0D,1D"]

print("")
print("GMRES number of iterations: ")
print(df_gmres.groupby(['nRanks']).mean()['nIterations_activationSolver'])

print("")
print("CG number of iterations: ")
print(df_cg.groupby(['nRanks']).mean()['nIterations_activationSolver'])

print_table(df_gmres, "Weak scaling on Hazel Hen", columns_to_plot, mylabels)
