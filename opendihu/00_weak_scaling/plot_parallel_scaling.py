...
######################
# create plot Node level
caption = "Node-level weak scaling, Hazel Hen,\n x,y,z=(12,8,2), xi=(20,2,2), 80 1D el./3D el. "
outfile = output_path+SCENARIO+'_weak_scaling_node_80.png'
plt.figure("node-level (80)", figsize=(10,8))
...
plt.xlabel('number of 1D elements (x1000)')
...
caption = "Multi-node weak scaling, Hazel Hen,\n x,y,z=(4,2,1), xi=(4,2,2), 16 1D el./3D el. "
outfile = output_path+SCENARIO+'_weak_scaling_multi_16.png'
plt.figure("multi-node (16)", figsize=(10,8))
...
for key in datasets:
  ...
  print dataset[3],dataset[4],dataset[5],nMperFE
...
xtickslist = sorted(list(set(xtickslist)))
xticks = [item[0] for item in xtickslist]
xlabels = [int(item[1])/48 for item in xtickslist]
#xlabels = [item[1]/48 for item in xtickslist]

ax2.set_xticks(xticks)
ax2.set_xticklabels(xlabels)
ax2.set_xlabel(r"Number of nodes (48 processes per node)")
...
######################
# create plot multi node
caption = "Multi-node weak scaling, Hazel Hen,\n x,y,z=(5,2,2), xi=(3,1,1), 3 1D el./3D el. "
outfile = output_path+SCENARIO+'_weak_scaling_multi_3.png'
plt.figure("multi-node (3)", figsize=(10,8))
